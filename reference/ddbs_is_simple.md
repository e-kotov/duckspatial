...
Checks if geometries are simple (no self-intersections) from a DuckDB
table using the spatial extension. Returns the result as an `sf` object
with a boolean simplicity column or creates a new table in the database.
...
a vector, an `sf` object with simplicity information or `TRUE`
