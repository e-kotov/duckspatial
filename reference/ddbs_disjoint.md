...
Tests if geometries in `x` are disjoint from geometries in `y`. Returns
`TRUE` if geometries have no points in common.
...
that are disjoint from the corresponding geometry in `x`. See
