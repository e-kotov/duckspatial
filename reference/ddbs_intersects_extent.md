...
Tests if the bounding box of geometries in `x` intersect the bounding
box of geometries in `y`. Returns `TRUE` if the extents (bounding boxes)
overlap. This is faster than full geometry intersection but less
precise.
...
whose bounding box intersects the bounding box of the corresponding
geometry in `x`. See
...
[`ddbs_predicate()`](https://cidree.github.io/duckspatial/reference/ddbs_predicate.md)
with `predicate = "intersects_extent"`.
...
#> 
#> $Portugal
#>  [1]  59  66  67  78  84  86  87  88  89  96  97  98  99 100
#> 
```
