...
Tests if geometries in `x` are properly within geometries in `y`.
Returns TRUE if geometry `x` is completely inside geometry `y` and does
not touch its boundary.
...
that properly contain the corresponding geometry in `x`. See
...
[`ddbs_predicate()`](https://cidree.github.io/duckspatial/reference/ddbs_predicate.md)
with `predicate = "within_properly"`.
